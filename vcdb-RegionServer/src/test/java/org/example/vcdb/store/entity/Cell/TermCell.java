...
public class TermCell {
    ...
    public byte[] toByteArray(){
        ...
        byte[] bytes=new byte[4+cf_name.getBytes().length+4+c_name.getBytes().length+4+max.getBytes().length+4+min.getBytes().length+4+equivalence.getBytes().length+4+like.getBytes().length];
        int pos=0;
